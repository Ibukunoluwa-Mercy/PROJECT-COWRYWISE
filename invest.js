...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (amountEl) {
        amountEl.innerHTML = `<span>₦</span>${formatNaira(getWalletBalance())}<span class="decimal" style="opacity: 0.5;">.00</span>`;
    }
    ...
    function startPaystackPayment() {
        ...
        const handler = PaystackPop.setup({
            ...
            callback(response) {
                ...
                if (amountEl) {
                    amountEl.innerHTML = `<span>₦</span>${formatNaira(newBalance)}<span class="decimal" style="opacity: 0.5;">.00</span>`;
                }
                closePaymentModal();
                Toastify({
